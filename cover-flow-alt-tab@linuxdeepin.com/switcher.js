...
function SwitchActor(app, window, _) {
    this._init(app, window, _);
}

SwitchActor.prototype = {
    _init : function(app, window, _) {
        this.app = app;
        this.window = window;
        this._ = _;
    ...
    getTitle: function() {
        if (this.isWorkspace) {
            ...
            return this._("Workspace ") + workspaceIndex;
        } else {
...
Switcher.prototype = {
    ...
    getSwitchActors: function() {
        ...
        for (let i = 0; i < apps.length; i++) {
            let windows = apps[i].get_windows();
            for(let j = 0; j < windows.length; j++) {
                let appIcon = new SwitchActor(null, windows[j], this._);
